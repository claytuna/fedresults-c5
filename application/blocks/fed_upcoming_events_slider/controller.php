<?php
namespace Application\Block\FedUpcomingEventsSlider;
...
{
    protected $btTable = 'btFedUpcomingEventsSlider';
    protected $btExportTables = array('btFedUpcomingEventsSlider', 'btFedUpcomingEventsSliderEntries');
    ...
    {
        return t("Choose upcoming events pages to feature in a slideshow format.");
    }

    public function getBlockTypeName()
    {
        return t("Events Slider - FedResults");
    }
    ...
    public function getSearchableContent()
    {
        ...
        $db = Database::get();
        $v = array($this->bID);
        $q = 'select * from btFedUpcomingEventsSliderEntries where bID = ?';
        $r = $db->query($q, $v);
        foreach ($r as $row) {
            $content .= $row['title'].' ';
    ...
    public function edit()
    {
        ...
        $this->requireAsset('redactor');
        $db = Database::get();
        $query = $db->GetAll('SELECT * from btFedUpcomingEventsSliderEntries WHERE bID = ? ORDER BY sortOrder', array($this->bID));
        $this->set('rows', $query);
    }
    ...
    {
        $db = Database::get();
        $r = $db->GetAll('SELECT * from btFedUpcomingEventsSliderEntries WHERE bID = ? ORDER BY sortOrder', array($this->bID));
    ...
    {
        $db = Database::get();
        $db->delete('btFedUpcomingEventsSliderEntries', array('bID' => $this->bID));
        parent::delete();
    }
    ...
    public function save($args)
    {
        ...
        $db = Database::get();
        $db->execute('DELETE from btFedUpcomingEventsSliderEntries WHERE bID = ?', array($this->bID));
        parent::save($args);
        if (isset($args['sortOrder'])) {
            ...
            while ($i < $count) {
                ...
                $db->execute('INSERT INTO btFedUpcomingEventsSliderEntries (bID, fID, title, description, sortOrder, linkURL, internalLinkCID) values(?, ?, ?, ?,?,?,?)',
                    array(
                        $this->bID,
                        intval($args['fID'][$i]),
                        $args['title'][$i],
